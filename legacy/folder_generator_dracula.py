...
class UniversalFolderCreator:
    ...
    def on_text_modified(self, event=None):
        if self.text_input.edit_modified():
            ...
            def apply_changes():
                ...
                max_count = 50
                ratio = min(count / max_count, 0.85)
            ...
            self._modified_after_id = self.master.after(100, apply_changes)
    ...
    def _run_import_task(self, anim_disabled=False):
        # 1. 目标路径计算
        today = datetime.today()
        year = today.strftime('%Y')
        ...
        with ThreadPoolExecutor(max_workers=len(tasks)) as ex:
            futures = [ex.submit(self._process_single_task, task) for task in tasks]
    ...
    def _process_single_task(self, task):
        ...
        anim_disabled = task.get("anim_disabled", False)
        ...
        if runner_canvas and not anim_disabled:
        ...
        try:
            ...
            for idx, filename in enumerate(files):
                ...
                dst_file = self.resolve_conflict(dst_dir, filename)
    ...
    def resolve_conflict(self, dst_dir, filename):
    ...
    def create_folders(self):
        ...
        revenue = shoots * self.PRICE_PER_SHOOT
